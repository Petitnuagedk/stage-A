...
# [1/18] Paquets requis
############################################
echo "[1/18] Installation des dépendances..."
step_start "[1/18] Installation des dépendances..."
...
############################################
# [2/18] Désactivation NetworkManager / WPA
############################################
echo "[2/18] Désactivation services conflictuels..."
step_start "[2/18] Désactivation services conflictuels..."
...
############################################
# [3/18] Activation systemd-networkd
############################################
echo "[3/18] Activation systemd-networkd..."
step_start "[3/18] Activation systemd-networkd..."

log "[ACTION] Activation systemd-networkd"
...
############################################
# [4/18] Synchronisation Date & Heure – Mesh Chrony
############################################
echo "[4/18] Configuration NTP Mesh (chrony)..."
step_start "[4/18] Configuration NTP Mesh (chrony)..."
...
############################################
# [5/18] Compilation et installation OLSRv2
############################################
echo "[5/18] Compilation OLSRv2 (OONF)..."
step_start "[5/18] Compilation OLSRv2 (OONF)..."
...
cmake .. -DCMAKE_INSTALL_PREFIX=/usr -DOONF_BUILD_PLUGINS=all
...
############################################
# [6/18] Configuration OLSRv2
############################################

echo "[6/18] Configuration OLSRv2..."
step_start "[6/18] Configuration OLSRv2..."
...
LoadPlugin "nhdp"
LoadPlugin "olsrv2"
LoadPlugin "olsrv2_lan"
LoadPlugin "kernel_routes"
LoadPlugin "telnet"

[interface=wlan0]

[olsrv2]
originator_validity_time 300.0
...
log "Configuration OLSRv2 avec telnet + forwarding activés"
step_ok
echo "Etape 6 OK."

############################################
# [7/18] Exclure wlan0 de systemd-networkd
############################################
echo "[7/18] Exclusion de wlan0 de systemd-networkd..."
step_start "[7/18] Exclusion de wlan0 de systemd-networkd..."
...
############################################
# [8/18] Passage en IP fiable
############################################
echo "[8/18] Script IBSS..."
step_start "[8/18] Script IBSS..."
...
############################################
# [9/18] Script IBSS – robuste avec logs
############################################
echo "[9/18] Script IBSS..."
step_start "[9/18] Script IBSS..."
...
iw dev wlan0 ibss join mesh-test 2437 fixed-freq

ip addr flush dev wlan0
ip addr add 10.0.0.$(cat /etc/mesh-id)/24 dev wlan0
ip link set wlan0 up
EOF
...
############################################
# [10/18] Service systemd – IBSS
############################################
echo "[10/18] Service systemd IBSS..."
step_start "[10/18] Service systemd IBSS..."
...
############################################
# [11/18] Ping broadcast périodique (FIXE)
############################################
echo "[11/18] Ping broadcast mesh (fixe)..."
step_start "[11/18] Ping broadcast mesh (fixe)..."
...
############################################
# [12/18] Service systemd – OLSRv2
############################################
echo "[12/18] Service systemd OLSRv2..."
step_start "[12/18] Service systemd OLSRv2..."

cat > /etc/systemd/system/olsrv2.service <<EOF
[Unit]
Description=OLSRv2 Routing Daemon
After=mesh-ibss.service
Wants=mesh-ibss.service
...
ExecStart=/usr/sbin/olsrd2_dynamic /etc/olsrd2/olsrd2.conf
Restart=on-failure
RestartSec=5
...
############################################
# [13/18] Script de vérification OLSRv2 post-boot
############################################
echo "[13/18] Script vérification OLSRv2 post-boot..."
step_start "[13/18] Script vérification OLSRv2 post-boot..."

cat > /usr/local/bin/mesh-check-olsr.sh <<'EOF'
#!/bin/bash

LOG="/var/log/mesh-olsr-check.log"
echo "=== Vérification OLSRv2 $(date) ===" >> "$LOG"
...
if ! systemctl is-active --quiet olsrv2.service; then
  echo "ERREUR: olsrv2.service inactif, redémarrage..." >> "$LOG"
  systemctl restart olsrv2.service
  sleep 5
fi

# Test telnet
if ! echo "/olsrv2info version" | nc -w 2 127.0.0.1 2006 > /dev/null 2>&1; then
  echo "ERREUR: Port telnet 2006 inaccessible" >> "$LOG"
  systemctl restart olsrv2.service
else
  echo "OK: Telnet OLSRv2 fonctionnel" >> "$LOG"
fi
...
chmod +x /usr/local/bin/mesh-check-olsr.sh

cat > /etc/systemd/system/mesh-check-olsr.service <<EOF
[Unit]
Description=Mesh OLSRv2 Post-Boot Check
After=olsrv2.service
Wants=olsrv2.service

[Service]
Type=oneshot
ExecStart=/usr/local/bin/mesh-check-olsr.sh
RemainAfterExit=yes
...
systemctl daemon-reload >>"$LOGFILE" 2>&1
systemctl enable mesh-check-olsr.service >>"$LOGFILE" 2>&1

step_ok
echo "Etape 13 OK."

############################################
# [14/18] Logs ICMP mesh JSON temps réel – déclenchement maître
############################################
echo "[14/18] Logs ICMP JSON temps réel..."
step_start "[14/18] Logs ICMP JSON temps réel..."
...
############################################
# [15/18] Script maître – Déclenchement ICMP global manuel
############################################
echo "[15/18] Préparation déclenchement ICMP global (manuel)..."
step_start "[15/18] Déclenchement ICMP global manuel"
...
############################################
# [16/18] Service ICMP JSON – durée limitée 600s
############################################
echo "[16/18] Configuration service ICMP JSON..."
step_start "[16/18] Service ICMP JSON"
...
############################################
# [17/18] Reset IP wlan0 après boot
############################################
echo "[17/18] Reset IP wlan0 après boot..."
step_start "[17/18] Reset IP wlan0 après boot..."
...
# [18/18] Identité & logs
############################################
echo "[18/18] Finalisation..."
step_start "[18/18] Finalisation..."
