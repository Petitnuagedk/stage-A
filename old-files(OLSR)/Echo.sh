...
# [1/14] Paquets requis
############################################
echo "[1/14] Installation des dépendances..."
step_start "[1/14] Installation des dépendances..."
...
############################################
# [2/14] Désactivation NetworkManager / WPA
############################################
echo "[2/14] Désactivation services conflictuels..."
step_start "[2/14] Désactivation services conflictuels..."
...
############################################
# [3/14] Activation systemd-networkd
############################################
echo "[3/14] Activation systemd-networkd..."
step_start "[3/14] Activation systemd-networkd..."
...
# ############################################
# # [4/14] Configuration réseau statique
# ############################################
# echo "[4/14] Configuration réseau wlan0..."
# step_start "[4/14] Configuration réseau wlan0..."
# 
...
############################################
# [5/14] Compilation et installation OLSRv2
############################################
echo "[5/14] Compilation OLSRv2 (OONF)..."
step_start "[5/14] Compilation OLSRv2 (OONF)..."
...
############################################
# [6/14] Configuration OLSRv2
############################################

echo "[6/14] Configuration OLSRv2..."
step_start "[6/14] Configuration OLSRv2..."
...
############################################
# [7/14] Exclure wlan0 de systemd-networkd
############################################
echo "[7/14] Exclusion de wlan0 de systemd-networkd..."
step_start "[7/14] Exclusion de wlan0 de systemd-networkd..."
...
############################################
# [8/14] Passage en IP fiable
############################################
echo "[8/14] Script IBSS..."
step_start "[8/14] Script IBSS..."
...
############################################
# [9/14] Script IBSS – robuste avec logs
############################################
echo "[9/14] Script IBSS..."
step_start "[9/14] Script IBSS..."
...
############################################
# [10/14] Service systemd – IBSS
############################################
echo "[10/14] Service systemd IBSS..."
step_start "[10/14] Service systemd IBSS..."
...
############################################
# [11/14] Ping broadcast périodique (FIXE)
############################################
echo "[11/14] Ping broadcast mesh (fixe)..."
step_start "[11/14] Ping broadcast mesh (fixe)..."
...
############################################
# [12/14] Service systemd – OLSRv2
############################################
echo "[12/14] Service systemd OLSRv2..."
step_start "[12/14] Service systemd OLSRv2..."
...
############################################
# [13/14] Logs ICMP mesh en temps réel
############################################
echo "[13/14] Logs ICMP temps réel..."
step_start "[13/14] Logs ICMP temps réel..."

if ! grep -q "MESH ICMP" /etc/nftables.conf; then
  sed -i '/chain input {/a \
        iifname "wlan0" ip protocol icmp limit rate 10/second log prefix "[MESH ICMP IN] " level info' /etc/nftables.conf

  sed -i '/chain output {/a \
        oifname "wlan0" ip protocol icmp limit rate 10/second log prefix "[MESH ICMP OUT] " level info' /etc/nftables.conf
fi

systemctl enable nftables >/dev/null 2>&1
systemctl restart nftables

cat > /usr/local/bin/mesh-ping-live <<'EOF'
#!/bin/bash
journalctl -k -f | grep "MESH ICMP" | awk '
{
  for (i=1; i<=NF; i++) {
    if ($i ~ /^SRC=/) src=$i
    if ($i ~ /^DST=/) dst=$i
  ...
  if ($0 ~ /MESH ICMP IN/)  dir="IN "
  if ($0 ~ /MESH ICMP OUT/) dir="OUT"

  printf "%s  %s  ->  %s\n\n", dir, src, dst
}'
EOF

chmod +x /usr/local/bin/mesh-ping-live

log "Logs ICMP mesh ACTIFS (kernel)"
log "Viewer temps réel : sudo mesh-ping-live"
step_ok
echo "Etape 13 OK."
...
# [14/14] Identité & logs
############################################
echo "[14/14] Finalisation..."
step_start "[14/14] Finalisation..."
