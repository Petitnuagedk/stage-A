...
############################################
echo "[2/12] Désactivation NetworkManager / WPA / Avahi"

systemctl disable --now NetworkManager 2>/dev/null || true
systemctl disable --now wpa_supplicant 2>/dev/null || true

# 🔥 FIX CRITIQUE : empêche IPv4LL (169.254.x.x)
systemctl disable --now avahi-daemon 2>/dev/null || true
systemctl mask avahi-daemon 2>/dev/null || true

############################################
# [3/12] systemd-networkd (eth0 uniquement)
############################################
...
ip addr add "${IP}/24" dev wlan0 noprefixroute
ip link set wlan0 up

echo "IBSS configuré avec IP ${IP}"
EOF

chmod +x /usr/local/bin/setup-adhoc.sh

############################################
# [8/12] Service mesh-ibss
############################################
