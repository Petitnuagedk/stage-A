...
if ! [[ "$BOARD_ID" =~ ^[0-9]+$ ]]; then
  echo "❌ BOARD_ID invalide: $BOARD_ID"
  exit 1
fi
...
# ↓ Désactivation temporaire de networkd sur wlan0
ip link set wlan0 down
sleep 0.5

# Configuration IBSS
echo "⚙️  Mise en place du mode IBSS..."
if ! iw dev wlan0 set type ibss; then
    echo "❌ Impossible de passer wlan0 en mode IBSS"
    exit 1
fi
...
if ! iw dev wlan0 ibss join "$ESSID" "$CHANNEL_FREQ" fixed-freq "02:11:22:33:44:$MAC_SUFFIX"; then
